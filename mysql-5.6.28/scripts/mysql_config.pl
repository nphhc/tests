...
#        handle  -lpthread -lm -lcrypt -ldl -lnuma  and other strings inserted that might contain
...
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lm -lcrypt -ldl -lnuma ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lm -lcrypt -ldl -lnuma ','',''];
